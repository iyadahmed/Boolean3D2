...
int main()
{
    ...
    triangle_t triangle = { .a = { { width / 2, 0, 0 } },
        .b = { { width, height, 0 } },
        .c = { { 0, height, 0 } } };
        ...
        for (j = 0; j < height; j++) {
            for (i = 0; i < width; i++) {
                if (use_antialiasing) {
                    ...
                    vec3_t sample_start = {
                        { (float)i + sample_spacing / 2, (float)j + sample_spacing / 2, 10 }
                    };
                    ...
                    for (int io = 0; io < 3; io++) {
                        for (int jo = 0; jo < 3; jo++) {
                            vec3_t offset = {
                                { io * sample_spacing, jo * sample_spacing, 0.0f }
                            };
                    ...
                    minimal_window_draw_pixel(
                        i, j, (uint8_t)(avg * 255.0f), 0, (uint8_t)(avg * 255.0f));
                } else {
                    ray_t ray = { .origin = { { (float)i + 0.5f, (float)j + 0.5f, 10 } },
                        .direction = { { 0, 0, -1 } } };
